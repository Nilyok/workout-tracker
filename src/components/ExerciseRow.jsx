...
export default function ExerciseRow({ row, onChange }) {
  ...
  return (
    <div className="px-2">
      {/* 💻 Desktop layout (completely hidden on mobile) */}
      <div className="hidden md:grid row-grid">
            ...
            <div className="font-medium">
              {row.videoUrl ? (
                <a
                  ...
                  onClick={(e) => { e.preventDefault(); console.log('Opening demo:', row.videoUrl); window.open(row.videoUrl, '_blank', 'noopener'); }}
                  role="link"
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); window.open(row.videoUrl, '_blank', 'noopener'); } }}
                >
                  {row.name}
                </a>
              ) : (
              ...
              <div className="text-xs text-slate-400 mt-1">Click name to view workout</div>
            </div>
          </div>
          ...
          {/* removed separate Watch Demo anchor; name is clickable instead */}
        </div>
        ...
        <div className="bg-slate-800/50 p-4 rounded-2xl mb-2 border border-slate-700/60 shadow-sm">
          {/* Header */}
            ...
            <div>
              ...
              <div className="text-xs text-slate-400 mt-1">Click the name to view the workout example</div>
            </div>
          ...
          <div className="space-y-3">
            ...
            />
            {/* video link is available by clicking the exercise name above */}
          </div>
        </div>
      </div>
    </div>
